...
> **最后更新**：2026-07-14（肉鸽分档：中 100–175 / 高 ≥176）
...
> **横轴（类型）**：§4 七类战斗定位 + 目录类型（消耗品 / 地形 / 未完成 / 待确认）。  
> **纵轴（职业）**：`plantTags` 中的**乐器职业**（鼓手、贝斯、吉他、键盘、主唱），**不是**乐队名（Mygo、AveMujica 等）。无乐器标签的 plant 归入「无职业标签」行。  
> **数据源**：§9.1 策划定稿 v1 定位 + `tools/plants_dump.json`（`python tools/list_plants.py`）。  
> **复现**：`python tools/build_profession_type_matrix.py` → `tools/profession_type_matrix.md`。
