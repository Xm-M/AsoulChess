...
## 功能描述
### 详细描述
多首的怪物新增**主动技能**：开启后将当前压力清零，消灭全部分身头，并按开技前头数 N 召唤 N 个**无占格、不可选中、不可被攻击**的轻量幽灵棋子。幽灵全屏找最近敌人，追击至攻击范围后近战持续攻击。开技后睦每秒压力 +X（默认 1，可配置），直到死亡；期间不再出新头、不再降低 `stressLimit`（保持开技时 limit）。**终局后本体禁止普攻与再次使用主动技**（仅保留 Blend=3 待机外观 + 加压自毁）。**终局幽灵近战**：在常规普攻之外，额外造成 **等于多首当前压力值** 的 **真实伤害**（压力 ≤0 时不追加）。**幽灵贴随**：进距/攻击中随目标位移并保持近战栓绳距离，避免站桩脱战。
